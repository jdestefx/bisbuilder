<?php
   ...
   class _siteSearch {
      ...
      function getZoneList() {
         $zonelist = $this->db->queryToObject("
      ...
      function fullItemSearch($filters) {
         ...
         $itemFields = "items.astr, items.adex, items.asta, items.acha, items.awis, items.aint,
            ...
            items.proceffect, items.magic, items.lore, items.nodrop, items.icon,
            ...
            items.worneffect, items.wornlevel, items.itemtype, items.focuseffect,
         ...
         // 2 = drop
         // 4 = quest
         $inventory = $this->db->queryToObject($qq = "
            select \"1\" as source, 
            ...
            select \"5\" as source,
            $itemFields, jeqTradeskillItems.itemID from 
            jeqTradeskillItems
            left join items on items.ID = jeqTradeskillItems.itemID
            $craftedConditions
      ...
      function getItemSourceDetails($itemIDs) {
         ...
         $merchant_data = $this->db->queryToObject("
            select
         ...
         $questitems = $this->db->queryToObject("
            select
            npc_types.id,
            npc_types.name,            
      ...
      function itemSearch($filters) {
         ...
         $q .= " group by itemID limit 1200";
         //and jeqItemsByZone.zone_short_name IS NOT NULL

         file_put_contents("/tmp/tril.log", "================================\n".$q."\n", FILE_APPEND);

         $res = $this->db->queryToObject($q);
         $itemIDs = [];

         for ($i=0;$i<sizeof($res);$i++) {
            $itemIDs[] = intval($res[$i]["itemID"]);
         }

         $newResults = $this->fullItemSearch([
            (object)Array("type"=>4, "itemIDs"=>$itemIDs)
         ]);

         //$searchResults = json_decode($this->db->json_encode_results($res));
         
         return array("query"=>$q, "searchResults"=>$newResults["inventory"]);

      }

      function getSpawnInfo($npc_name) {

         $spawnInfo = $this->db->query("
            select
         ...
         return array(
            "spawnInfo"=>json_decode($this->db->json_encode_results($spawnInfo))
         );
